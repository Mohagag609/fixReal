  ...
  try {
    ...
    if (multipleSheets && Array.isArray(multipleSheets)) {
      ...
      if (asZip) {
        ...
        response = new NextResponse(buffer as BodyInit)
        response.headers.set('Content-Type', 'application/zip')
        response.headers.set('Content-Disposition', `attachment; filename="reports-${new Date().toISOString().split('T')[0] || 'غير محدد'}.zip"`)
      ...
      const csv = exportToCSV({
        ...
        fileName: fileName || `report-${new Date().toISOString().split('T')[0] || 'غير محدد'}.csv`
      })
...
export async function GET(request: NextRequest) {
  try {
    ...
    const csv = exportToCSV({
      ...
      data: sampleData,
      reportType,
      fileName: `${reportType}-report-${new Date().toISOString().split('T')[0] || 'غير محدد'}.csv`
    })
